...
This repository started from a practical industrial inspection question: what defect detection workflow is most reliable when normal images are plentiful but labeled defect examples are sparse, incomplete, or unrepresentative?
...
Vision-language models were explored next, not because they were expected to replace anomaly detection, but because they offered a different kind of signal: semantic similarity and prompt-guided reasoning. WinCLIP did contribute useful recall behavior, but its latency made it difficult to position as a standalone production choice in this benchmark.
...
The core lesson from the project is simple: in inspection systems, recall and false-negative rate are operationally more important than a single headline AUROC value. The benchmark therefore treats model selection as an engineering tradeoff problem, not just a ranking exercise.
...
There are a few adjacent folders for deployment, API, and container ideas, but the completed work in this repo is the MVTec benchmark itself.
