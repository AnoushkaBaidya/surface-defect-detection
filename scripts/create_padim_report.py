"""
Create the PaDiM benchmark report.

Outputs:
- per_category_results.csv
- aggregate_results.csv
- padim_benchmark_report.md
"""
...
def load_metrics(raw_metrics_dir: Path) -> pd.DataFrame:
    """Load all PaDiM metrics JSON files."""
    rows = []

    for metrics_file in sorted(raw_metrics_dir.glob("*_padim_metrics.json")):
...
def create_markdown(per_category_df: pd.DataFrame, aggregate_df: pd.DataFrame) -> str:
    ...
    markdown = [
        "# PaDiM benchmark — PaDiM All-Category Benchmark Report",
        "",
        "## Purpose",
        "",
        "This report summarizes PaDiM normal-only anomaly detection performance across all 15 MVTec AD categories.",
        "",
        "PaDiM is evaluated as an unsupervised/category-specific anomaly detector trained only on normal images.",
        "",
        ...
        "- Tuning focus: backbone, layers, image size, thresholding, latency, and localization quality",
        "",
        "## Metric Notes",
        "",
        "- `classification_auroc` = image-level AUROC.",
        "- `segmentation_auroc` = pixel-level AUROC.",
        "- Recall/FNR are included when prediction labels are available from the installed Anomalib version.",
        "",
        ...
        "",
        "- PaDiM provides both classification and localization metrics.",
        "- It should be compared against ResNet18 on missed defects and against later Anomalib models on segmentation quality.",
        "- Categories with high segmentation AUROC but low segmentation F1 may require better thresholding.",
        "",
    ]
...
def main() -> int:
    ...
    if per_category_df.empty:
        raise FileNotFoundError(f"No PaDiM metrics found in {raw_metrics_dir}")
