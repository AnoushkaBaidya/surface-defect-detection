"""
Train/evaluate PatchCore on one MVTec AD category.

Purpose
-------
This script runs PatchCore using Anomalib and exports standardized benchmark
results aligned with the shared benchmark contract.

Engineering note
----------------
PatchCore is a normal-only memory-bank anomaly detection method. It extracts
patch features from normal images, stores a representative coreset, and scores
test patches by nearest-neighbor distance.

Therefore, industrial tuning focuses on:
- backbone
- feature layers
...
def save_model_artifact(engine, model, model_dir: Path) -> str | None:
    """
    Save PatchCore model artifact when supported.
    ...
    checkpoint_path = model_dir / "patchcore_model.ckpt"
    ...
    try:
        torch.save(model.state_dict(), model_dir / "patchcore_state_dict.pt")
        return str((model_dir / "patchcore_state_dict.pt").relative_to(PROJECT_ROOT))
    except Exception as error:  # noqa: BLE001
        logger.warning("Could not save PatchCore state_dict: %s", error)

    return None


def build_patchcore_model(config: dict):
    """
    Build PatchCore model with version-tolerant constructor handling.

    Anomalib constructor signatures can vary between versions. The code first tries the
    explicit config. If unsupported, the implementation falls back safely to default
    PatchCore so the benchmark can still run.
    """
    ...
    try:
        ...
        logger.warning("Installed Patchcore constructor did not accept full config.")
        logger.warning("Trying lowercase PatchCore-compatible defaults.")
    ...
    try:
        ...
        logger.warning("Installed Patchcore constructor did not accept backbone/layers.")
        logger.warning("Falling back to default Patchcore().")
...
def main() -> int:
    """
    Run PatchCore for one category.
    """
    parser = argparse.ArgumentParser(description="Train/evaluate the PatchCore benchmark.")
    ...
    logger.info("Running PatchCore category=%s accelerator=%s", category, accelerator)
    ...
    model = build_patchcore_model(config)
    ...
    with mlflow.start_run(run_name=f"patchcore_benchmark_{category}"):
        mlflow.log_params(
            {
                "category": category,
        ...
        benchmark_result = BenchmarkResult(
            run_id=f"anomalib_patchcore_{category}_seed{seed}",
            model_family="anomalib",
            model_name="patchcore",
            category=category,
            seed=seed,
            ...
            notes=(
                "PatchCore normal-only anomaly detection. Recall/FNR are extracted "
                "from Anomalib predictions when prediction labels are available."
            ),
        )
        ...
        metrics_path = raw_metrics_dir / f"{category}_patchcore_metrics.json"
